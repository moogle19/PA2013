...
public class CLUtil 
{
    ...
    public static long measureKernelCall(CLKernel kernel, CLCommandQueue queue, PointerBuffer gws, PointerBuffer lws)
    {
        PointerBuffer event = new PointerBuffer(1);
        
        OpenCL.clEnqueueNDRangeKernel(queue, kernel, 1, null, gws, lws, null, event);
        
        OpenCL.clFlush(queue);
        
        OpenCL.clWaitForEvents(event);
        
        ByteBuffer start = BufferUtils.createByteBuffer(8);
        ByteBuffer end = BufferUtils.createByteBuffer(8);
        
        CLEvent clEvent = queue.getCLEvent(event.get(0));
        
        OpenCL.clGetEventProfilingInfo(clEvent, OpenCL.CL_PROFILING_COMMAND_START, start);
        OpenCL.clGetEventProfilingInfo(clEvent, OpenCL.CL_PROFILING_COMMAND_END, end);
        
        OpenCL.clReleaseEvent(clEvent);
        
        return end.getLong() - start.getLong();
    }
}
