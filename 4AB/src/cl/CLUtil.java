...
public class CLUtil 
{
    ...
    public static void printProgramInfo(CLProgram program, CLDevice device) 
    {
        ...
        // status
        PointerBuffer statusSize = BufferUtils.createPointerBuffer(1);
        err = CL10.clGetProgramBuildInfo(program, device, CL10.CL_PROGRAM_BUILD_STATUS, null, statusSize);
        ByteBuffer status = BufferUtils.createByteBuffer((int)statusSize.get(0));
        CL10.clGetProgramBuildInfo(program, device, CL10.CL_PROGRAM_BUILD_STATUS, status, statusSize);
        byte statusBytes[] = new byte[status.capacity()];
        status.get(statusBytes);
        int stat = status.getInt(0);
        System.out.println(String.format("{Status= %d},", stat));
        // options
        PointerBuffer optionsSize = BufferUtils.createPointerBuffer(1);
        CL10.clGetProgramBuildInfo(program, device, CL10.CL_PROGRAM_BUILD_OPTIONS, null, optionsSize);
        ByteBuffer options = BufferUtils.createByteBuffer((int)optionsSize.get(0));
        CL10.clGetProgramBuildInfo(program, device, CL10.CL_PROGRAM_BUILD_OPTIONS, options, optionsSize);
        byte opt = options.get(0);
        System.out.println(String.format("{Options= %s},", opt));
        //error log
        PointerBuffer logSize = BufferUtils.createPointerBuffer(1);
        CL10.clGetProgramBuildInfo(program, device, CL10.CL_PROGRAM_BUILD_LOG, null, logSize);
        if(logSize.get(0) > 2) {
            ByteBuffer log = BufferUtils.createByteBuffer((int)logSize.get(0));
            CL10.clGetProgramBuildInfo(program, device, CL10.CL_PROGRAM_BUILD_LOG, log, logSize);
            byte logBytes[] = new byte[log.capacity()];
            log.get(logBytes);
            System.out.println(String.format("[ErrorLog=\n %s]", new String(logBytes)));
        }
        checkError(err);
    }
